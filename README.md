...
### 5.4 Supporting Property: Non-Recomputational Forensic Analysis

**Problem in traditional trace analysis**

Many trace analysis systems perform lazy recomputation during interactive exploration. Operations such as filtering, zooming, or aggregation trigger:
- Recomputation of derived metrics (statistics, clustering)
- Changes in statistical summaries and aggregates
- Invalidation or update of cached results
- Uncertainty about whether displayed values are fresh or stale

As a result, the analytical state is not strictly stable under interaction, and reproducibility of intermediate analytical views is not guaranteed.

**Design principle: frozen analytical state**

The Observatory separates three layers:

1. **Trace data layer (immutable log):** Raw execution events, frozen at load
2. **Forensic inference layer (precomputed consensus model):** Clustering, operator attribution, temporal projection—computed once at load, never recomputed
3. **Interactive visualization layer (pure projection):** UI state mutations only; no computation

Once forensic inference is computed, it is never touched during interaction. All interactive operations (zoom, filter, cluster selection, snapshot export) only select *views over precomputed results*. No UI action triggers re-clustering, re-attribution, or re-projection.

**Consequence**

All of the following are provably invariant under arbitrary interaction:
- Divergence clustering results (same clusters, same boundaries)
- Operator attribution values (same operator intensities)
- Temporal dependency projections (same backtrace edges)
- Snapshot exports (bitwise identical across exports)

**Implication**

This ensures that interactive exploration is non-destructive with respect to forensic interpretation. The analyst can explore freely without corrupting or unknowingly changing the underlying analysis.

**Prior art context**

Immutable data under interactive querying is established in:
- Event sourcing systems (immutable logs + many read models)
- OLAP systems (frozen datasets + interactive pivot queries)
- Reproducible computing notebooks (snapshot + re-query model)
- Some snapshot-based trace visualization tools

What distinguishes this system: immutability extends to *derived forensic semantics*, not just raw data. The inference results (what the data means) are precomputed and frozen, not the data itself. Most systems freeze the source; this system freezes the interpretation.

---

### 5.5 Contribution Hierarchy (Clarity on What Is Novel)

This section formalizes the contribution structure for peer review clarity.

**Tier 1: Core Contributions (Defensible novelty)**

1. **Type-enforced immutability boundary for forensic inference layer**
   - Rust type system enforces that forensic data cannot be mutated from any code path
   - Structural guarantee, not design pattern
   - Enables forensic reproducibility guarantees
   - Underexplored in kernel trace analysis (though immutability itself is established)

2. **Post-hoc latent decomposition of trace-derived signals**
   - Infer operator contributions from energy metrics without instrumentation
   - Trade accuracy for zero overhead (no probes needed)
   - Model validated internally on consistency; not validated against ground truth
   - Practical utility for non-invasive operator attribution

3. **Deterministic temporal dependency projection**
   - Fixed lag-window correlation for reproducible trace exploration
   - Transparent, non-learned heuristics (reproducibility > optimality trade-off)
   - Clearly stated as correlation (temporal precedence), not causation
   - Enables interactive exploration without interpretation drift

**Tier 2: Supporting Contributions (Clarifying properties)**

4. **Non-recomputational forensic analysis model**
   - All inference precomputed at load; interaction never triggers recomputation
   - Ensures analytical state is stable under exploration
   - Consequence of Tier 1 + Tier 2 design, not novel in principle
   - Clarifies why reproducibility is achieved in practice

**Tier 3: Engineering/Practical Contributions (System-level)**

5. **Reproducibility-first architecture for kernel trace analysis**
   - Design discipline prioritizing forensic reproducibility over observability
   - Appropriate trade-off for development/audit contexts
   - Demonstrates feasibility of forensic-grade trace systems in Rust

**NOT claimed as novel:**

- ✗ Immutability as a principle (established in functional programming)
- ✗ Immutable data storage (standard in event sourcing)
- ✗ Interactive querying over frozen data (OLAP, notebooks)
- ✗ Operator signal decomposition (PCA, NMF, factor analysis)
- ✗ Temporal correlation analysis (basic statistics)
- ✗ Type-system enforcement (Rust language feature)

---

### 5.6 Honest Debate: What Is Actually Novel?

This work's novelty is in *systems architecture and forensic design priorities*, not in mathematical or algorithmic breakthroughs. The contributions are:

1. **Architectural pattern:** Immutable forensic inference layer + mutable UI layer with no feedback path
2. **Engineering discipline:** Reproducibility-first design for trace analysis (rare in observability tools)
3. **Practical applicability:** Works on kernel traces without instrumentation; Rust type system enforces guarantees

These are legitimate systems contributions appropriate for a systems/architecture venue, but not fundamental research advances in kernel analysis, causal inference, or statistics.

**Realistic scope:** This is an incremental but solid systems paper. It takes established techniques (immutability, separation of concerns, precomputation) and combines them into a forensic-grade system. The novelty is in the combination and application, not in any individual component.
...
**Distinction:** All of these req
uire either instrumentation, code analysis, or formal model. The Observatory trades accuracy for non-invasiveness (post-hoc analysis of existing traces).
